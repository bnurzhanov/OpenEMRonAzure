...
if [ "$(ls -A "$VOL" 2>/dev/null)" ]; then
  ...
  # Even if already populated, ensure sqlconf.php exists / is updated.
  if [ ! -f "$VOL/default/sqlconf.php" ]; then
    echo "[seed-sites] Existing volume missing sqlconf.php; generating.";
  else
    echo "[seed-sites] Existing volume already has sqlconf.php; leaving in place.";
    touch "$VOL/.seeded"; exit 0; fi
fi
echo "[seed-sites] Copying OpenEMR sites skeleton into persistent volume..."
cp -a /var/www/localhost/htdocs/openemr/sites/* "$VOL"/

# Always (re)generate dynamic sqlconf.php so credentials come from env at runtime.
CONF="$VOL/default/sqlconf.php"
echo "[seed-sites] Writing dynamic sqlconf.php..."
cat > "$CONF" <<'PHP'
<?php
// Auto-generated by Azure Container Apps init. Uses container environment variables.
...
// Keep file minimal; advanced options can be appended manually if needed.
?>
PHP
